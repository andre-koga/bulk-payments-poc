...
def seed_demo_dataset(conn: sqlite3.Connection) -> dict[str, list[str]]:
    """
    Insert tenants, vendors, bills, payments and FX rates including:
    - clear bulk match (unique subset) — USD only
    - ambiguous subsets (same sum) → suggest only
    - noise bills outside date window
    - EUR bill with FX rate (EUR→USD, t1): pay_fx_eur matches one EUR bill after normalization
    - missing FX scenario: EUR bill with no rate → skipped during retrieval (fail closed)
    """
    init_db(conn)
    cur = conn.cursor()
    ...
    # EUR bill: €90 @ 1.10 USD/EUR = $99 USD — used in pay_fx_eur
    bills_t1.append(("b_eur", "t1", "v_acme", _eur_minor(90), "EUR", base.isoformat(), None, None))
    # EUR bill with no FX rate available (should be silently skipped by retrieval)
    bills_t1.append(("b_eur_nofx", "t1", "v_acme", _eur_minor(200), "EUR", base.isoformat(), None, None))
    ...
    payments = [
        ...
        # FX scenario: €270 payment from ACME bank → expect EUR bills summing to €270
        # We seed b1(100 USD)+b2(250 USD)+b_eur(90 EUR@1.10=99 USD) doesn't cleanly hit 270 USD.
        # Instead: payment is $198 USD which matches 2x b_eur(€90@1.10=99 USD each) if we had two.
        # Simpler: payment $99 USD should uniquely match b_eur (€90 @ 1.10).
        (
            "pay_fx_eur",
            "t1",
            _usd_minor(99),  # $99 USD — matches b_eur after FX normalization
            pay_day.isoformat(),
            "Acme National",
            "EUR invoice",
            1.0,
    ...
    # Seed FX rates for EUR→USD for tenant t1 (used by pay_fx_eur and b_eur)
    # b_eur_nofx is EUR but has no rate → retrieval will skip it
    upsert_fx_rate(
        conn,
        tenant_id="t1",
        from_currency="EUR",
